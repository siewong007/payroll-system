...
fn render_payslip_page(data: &PayslipData, company: &CompanyInfo) -> AppResult<Vec<u8>> {
    ...
    if let Some(ref a) = company.address_line1 { addr_parts.push(a.clone()); }
    if let Some(ref a) = company.address_line2 { addr_parts.push(a.clone()); }
    ...
    if !city_parts.is_empty() { addr_parts.push(city_parts.join(", ")); }
    ...
    // Payslip title
    add_text(&mut ops, &bold, 12.0, left, y, &format!("PAYSLIP - {} {}", month_name, data.period_year));
    y -= 8.0;

    // Employee info
    add_text(&mut ops, &font, 9.0, left, y, &format!("Employee: {}", data.employee_name));
    add_text(&mut ops, &font, 9.0, 110.0, y, &format!("Employee No: {}", data.employee_number));
    y -= 5.0;
    ...
    if let Some(ref dept) = data.department {
        add_text(&mut ops, &font, 9.0, 110.0, y, &format!("Department: {}", dept));
    }
    y -= 5.0;
    if let Some(ref desig) = data.designation {
        add_text(&mut ops, &font, 9.0, left, y, &format!("Designation: {}", desig));
    }
    add_text(&mut ops, &font, 9.0, 110.0, y, &format!("Pay Date: {}", data.pay_date));
    y -= 5.0;
    add_text(&mut ops, &font, 9.0, left, y, &format!("Pay Period: {} to {}", data.period_start, data.period_end));
    y -= 4.0;
    ...
    for (label, amount) in &earnings {
        if *amount > 0 {
            draw_row(&mut ops, &font, &bold, 9.0, left + 5.0, right, y, label, &sen_to_rm(*amount), false);
            y -= 5.0;
        }
    }
    draw_line(&mut ops, left, right, y + 1.0);
    y -= 5.0;
    draw_row(&mut ops, &font, &bold, 9.0, left, right, y, "TOTAL EARNINGS", &sen_to_rm(data.gross_salary), true);
    y -= 8.0;
    ...
    if data.total_claims > 0 {
        ...
        y -= 6.0;
        draw_row(&mut ops, &font, &bold, 9.0, left + 5.0, right, y, "Reimbursements", &sen_to_rm(data.total_claims), false);
        y -= 5.0;
        draw_line(&mut ops, left, right, y + 1.0);
        y -= 5.0;
        draw_row(&mut ops, &font, &bold, 9.0, left, right, y, "TOTAL PAYMENTS", &sen_to_rm(data.total_claims), true);
        y -= 8.0;
    }
    ...
    for (label, amount) in &deductions {
        if *amount > 0 {
            draw_row(&mut ops, &font, &bold, 9.0, left + 5.0, right, y, label, &format!("-{}", sen_to_rm(*amount)), false);
            y -= 5.0;
        }
    }
    draw_line(&mut ops, left, right, y + 1.0);
    y -= 5.0;
    draw_row(&mut ops, &font, &bold, 9.0, left, right, y, "TOTAL DEDUCTIONS", &format!("-{}", sen_to_rm(data.total_deductions)), true);
    y -= 10.0;

    // Net Salary
    draw_line(&mut ops, left, right, y + 2.0);
    y -= 2.0;
    draw_row(&mut ops, &font, &bold, 11.0, left, right, y, "NET SALARY", &format!("RM {}", sen_to_rm(data.net_salary)), true);
    y -= 4.0;
    draw_line(&mut ops, left, right, y);
    y -= 10.0;
    ...
    if let (Some(bank), Some(acc)) = (&data.bank_name, &data.bank_account_number) {
        add_text(&mut ops, &font, 8.0, left, y, &format!("Paid by bank transfer: {} (Acc: {})", bank, acc));
        y -= 8.0;
    }
    ...
    y -= 6.0;
    draw_row(&mut ops, &font, &bold, 9.0, left + 5.0, right, y, "EPF (Employer)", &sen_to_rm(data.epf_employer), false);
    y -= 5.0;
    draw_row(&mut ops, &font, &bold, 9.0, left + 5.0, right, y, "SOCSO (Employer)", &sen_to_rm(data.socso_employer), false);
    y -= 5.0;
    draw_row(&mut ops, &font, &bold, 9.0, left + 5.0, right, y, "EIS (Employer)", &sen_to_rm(data.eis_employer), false);
    y -= 5.0;
    draw_line(&mut ops, left, right, y + 1.0);
    y -= 5.0;
    draw_row(&mut ops, &font, &bold, 9.0, left, right, y, "TOTAL EMPLOYER COST", &sen_to_rm(data.employer_cost), true);
    y -= 10.0;

    // YTD Summary
    add_text(&mut ops, &bold, 10.0, left, y, &format!("YEAR-TO-DATE (Jan - Dec {})", data.period_year));
    y -= 6.0;
    ...
    // Footer
    add_text(&mut ops, &font, 7.0, left, 15.0, "This is a computer-generated payslip. No signature is required.");
...
fn render_payslip_ops(data: &PayslipData, company: &CompanyInfo, font: &PdfFontHandle, bold: &PdfFontHandle) -> PdfPage {
    let mut ops: Vec<Op> = Vec::new();
    let left = 20.0;
    let right = 190.0;
    let mut y = 275.0;
