# CodeGraph reference: extra exports and benchmark
...
```bash
./scripts/codegraph export neo4j
```

**If `--neo4j-push <uri>`** - push directly to a running Neo4j instance. Ask the user for credentials if not provided:

```bash
./scripts/codegraph export neo4j --push bolt://localhost:7687 --user neo4j --password PASSWORD
```
...
```bash
./scripts/codegraph export falkordb
```
...
```bash
./scripts/codegraph export falkordb --push falkordb://localhost:6379
```
...
```bash
./scripts/codegraph export svg
```

### Step 7c - GraphML export (only if --graphml flag)

```bash
./scripts/codegraph export graphml
```
