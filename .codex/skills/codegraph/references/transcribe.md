...
**Strategy:** If `graphify-out/.graphify_analysis.json` exists from a previous run, read its god-node labels; otherwise infer the domain from the file names and paths in `graphify-out/.graphify_detect.json` (detect output has no god nodes — those only exist after a build). You are already a language model — write a one-sentence domain hint yourself from what you find. Then pass it to Whisper as the initial prompt. No separate API call needed.

**However**, if the corpus has *only* video files and no other docs/code, use the generic fallback prompt: `"Use proper punctuation and paragraph breaks."`

**Step 1 - Write the Whisper prompt yourself.**

Read the top god node labels from a prior run's analysis (or infer the domain from detected file names), then compose a short domain hint sentence, for example:
